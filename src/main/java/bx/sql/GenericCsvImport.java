...
public class GenericCsvImport implements CsvImport<GenericCsvImport> {
  ...
  public static GenericCsvImport into(DataSource dataSource) {

    GenericCsvImport imp = new GenericCsvImport();
    imp.dataSource = dataSource;
    return imp;
  }

  public GenericCsvImport from(File file) {
  ...
  public GenericCsvImport from(ByteSource source) {
    this.byteSource = source;
    return this;
  }

  public GenericCsvImport fromString(String data) {
    this.byteSource = ByteSource.wrap(data.getBytes(StandardCharsets.UTF_8));
    return this;
  }

  public GenericCsvImport into(String table) {
    return table(table);
  }

  public GenericCsvImport table(String table) {
    this.table = table;
    return this;
  }
  ...
  private void collectTableMetadata() {
    ...
    BxJdbcClient.create(dataSource, table)
        .sql(sql)
        .query(
            rse -> {
  ...
  public GenericCsvImport gzip(boolean gz) {
    this.gzip = gz;
    return this;
  }
    ...
    try (Defer defer = Defer.create()) {
      ...
      while (t.hasNext()) {
        ...
        if (sql == null) {
          ...
          statementSpec = BxJdbcClient.create(dataSource, table).sql(sql);
        }
